package cernoch.scalistics.aggregator.single

class MEAN[T,N]
  ( private[aggregator] var sum: SUM[T],
    private[aggregator] var cnt: SUM[N] )
  ( implicit
    frac: Fractional[T],
    num: Numeric[N],
    conv: N => T )
...
class MeanDoubleExact[N]
  (private[aggregator] val mean: MEAN[BigDecimal,N])
